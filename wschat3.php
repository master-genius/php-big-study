...
class wsChat
{
    ...
    public function on_message($server, $cnn)
    {
        ...
        $send_msg = $this->format_groupmsg($msg,'text',time(),$this->getUserByConn($cnn->fd));
    ...
    public function on_open($server, $req)
    {
        if (!$this->checkUser($req->get['user_token'])) {
            ...
            $server->push(
                $req->fd,
                $this->format_sysmsg('you need to login','server','','text',-1)
            );
            $server->close($req->fd);
        }
        ...
        $sys_msg = $this->format_sysmsg('you are login at '.  $req->fd, 'server', '','text');
